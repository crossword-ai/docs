---
title: "Orchestration"
description: "How channels, experiences, playbooks, rulebooks, and campaigns decide what a shopper sees"
---
...
- **Inside the experience.** Playbooks propose candidates for each rule-based component type. Rulebooks resolve the proposals into what shows. The Default Playbook supplies the fallback. Campaigns run over time and can activate a Playbook for a shopper's next visit.
- **Audiences.** Shared segments that Campaigns enroll and that rules on components can test against.
- **Experiments.** Act on Playbooks, not on the experience. An experiment splits traffic between two or more Playbooks and measures a goal event.
...
<CardGroup cols={2}>
  ...
  <Card title="Rulebooks" href="/orchestration/rulebooks">
    Where proposals from every Playbook are resolved.
  </Card>
  ...
  <Card title="Experiments" href="/orchestration/experiments">
    Split traffic between Playbooks and measure a goal.
  </Card>
