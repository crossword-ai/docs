---
title: "Playbooks vs campaigns"
description: "When immediate contextual behavior is enough and when you need a stateful sequence"
---

Playbooks and Campaigns both change what a shopper sees. They answer different questions. A Playbook answers "what should happen right now, given this context?" A Campaign answers "what should happen to this person over the next hours or days?"

## The short version

A **Playbook** is a reusable collection of contextual behaviors that Crossword applies when relevant. It has no memory of the shopper's progress. Every rule inside it is evaluated fresh against the current context.
...
If the behavior depends only on what is true right now, use a Playbook. If the behavior depends on what already happened and what should happen next, use a Campaign.

## A Playbook in practice

Take a Playbook called YouTube Unboxing. It exists to serve shoppers who arrive from an unboxing video. It does not switch on as a whole. Instead, each rule-based component inside it carries its own rule.
...
The screenshot shows the YouTube Unboxing Playbook open on the **Playbooks** screen with its Enabled toggle on. The components list has the six rows from the table above. Each row shows the component type, its label, and its rule in one line, such as the welcome message with Source is YouTube Unboxing A and the proactive message with 15 seconds of inactivity. No condition appears on the Playbook itself.

<Frame caption="YouTube Unboxing Playbook components and rules">
  <img src="/images/placeholder.svg" alt="YouTube Unboxing Playbook components and rules" />
</Frame>
...
```mermaid
flowchart TB
  A["Shopper has ₹12,000 in cart"] --> B["Wait 30 minutes"]
  B --> C{"Purchased?"}
  C -- Yes --> X1["Exit"]
  C -- No --> D["Send message"]
  D --> E["Wait 6 hours"]
...
This is not a Playbook. It has memory of where the shopper currently is in the sequence. It waits, checks, branches, and continues. That memory is what makes it a Campaign.
...
**Contextual proactive message.** This belongs in a Playbook's proactive-message rules.
...
## Campaigns and Playbooks work together

A Campaign step can activate a Playbook. Cart Recovery might apply an "Abandoned Cart Return Visit" Playbook on the shopper's next visit. The Campaign handles the timing and the memory. The Playbook handles what the shopper sees when they land. Neither has to do the other's job.
...
| You want to | Use |
| ----------- | --- |
| Change the welcome message for visitors from an ad | Playbook |
| Show different starters on a product page | Playbook |
| Nudge a shopper who has been idle for 15 seconds | Playbook |
...
| Change what a returning shopper sees when a Campaign brings them back | Campaign that activates a Playbook |
