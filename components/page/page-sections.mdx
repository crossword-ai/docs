...
| [Offer section](/components/page/offer-section) | An offer, who qualifies, an explanation, and a call to action. |

## The shared rule shape

Each type holds its own content. All six carry the same rule, which says where the section goes and when it is a candidate.
...
1. Every enabled Playbook contributes the sections whose target page and placement match the current page.
...
A page can have several placements, and each shows its own winner, so sections from different Playbooks can sit on the same page. When no rule matches, the Default Playbook's section for that placement shows. If it has none, the storefront's own content stays as it is. Read [How rulebooks resolve conflicts](/orchestration/how-rulebooks-resolve).
