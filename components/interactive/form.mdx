...
flowchart TB
  ...
  FM --> TYPE["Form type<br/><small>contact collection, or any fields you define</small>"]
  FM --> FIELDS
  FM --> CONSENT["Consent fields<br/><small>marketing consent or terms</small>"]
  FM --> SUBMIT["Submit control<br/><small>such as Notify me</small>"]
  FM --> MSG
  subgraph FIELDS["Fields"]
    F["Field"]
    F --> LB["Label"]
    F --> IT["Input type<br/><small>text, email, phone, or number</small>"]
    F --> RQ["Required"]
    F --> VA["Validation"]
  end
  subgraph MSG["Success and error messages"]
    OK["Success message"]
    ERR["Error message<br/><small>validation fails or submission cannot complete</small>"]
  end
  PRIM["Primitives<br/><small>input and button</small>"]
  IT -. "built from the input" .-> PRIM
  SUBMIT -. "built from the button" .-> PRIM
  WF["Workflow<br/><small>what happens with the submitted values</small>"]
  SUBMIT -. "on submit" .-> WF
```
...
A form carries no rule. It shows because the welcome message, or a proactive message or page section with a rule, included it, or because the Agent composed it into the assistant reply. The rule that decides whether and when the shopper sees it lives on that component, not on the form. The welcome message is the exception: it carries no rule and is global, so every shopper sees what it embeds. Read [Rules](/orchestration/rules).
